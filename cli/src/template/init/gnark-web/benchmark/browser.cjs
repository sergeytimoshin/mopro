...
(async () => {
    ...
    if (!["go", "portable"].includes(mode)) throw new Error("Unknown MOPRO_GNARK_MODE");
    ...
    try {
        ...
        const result = await driver.executeAsyncScript(function (mode, base, done) {
            (async () => {
                ...
                const start = await timed(() => api.initGnark());
                ...
                return { userAgent: navigator.userAgent, mode, isolated: crossOriginIsolated,
                    hardwareConcurrency: navigator.hardwareConcurrency, fixture,
