...
p=argparse.ArgumentParser();p.add_argument('root',type=Path);p.add_argument('stage',choices=['checks','bench','workers','keys']);p.add_argument('--verifier',type=Path,required=True);p.add_argument('--port',type=int,default=3217);a=p.parse_args()
...
def run(variant,fixture,threads,session,count=31):
 name=f'{a.stage}-{fixture}-{variant}-{threads}-{session}';report=reports/(name+'.json');web=r/variant/'web';g=web/'MoproWasmBindings/gnark'
 env=dict(base,MOPRO_GNARK_MODE='rust',MOPRO_GNARK_THREADS=str(threads),MOPRO_GNARK_BENCH_BACKEND=variant,MOPRO_GNARK_BENCH_SAMPLES=str(count),MOPRO_GNARK_BENCH_WARMUPS='5',MOPRO_GNARK_BENCH_URL=f'http://127.0.0.1:{a.port}/{variant}/web/gnark-benchmark.html',MOPRO_GNARK_BENCH_BASE=f'./assets/{fixture}/',MOPRO_GNARK_BENCH_REPORT=str(report))
 ...
 result=json.loads(report.read_text());assert result['backend']==variant;assert result['threads']==threads
...
if a.stage=='checks':
 ...
 for fixture in ['square-raw','mimc-raw','commitments-raw']:
  for threads in [1,2,4,8,16]:run('arkworks',fixture,threads,1,9)
else:
 for session in range(1,4):
