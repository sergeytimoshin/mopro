...
The browser adapter uses **gnark 0.14.0 / gnark-crypto 0.19.0**. The native mobile backend on the `feat/gnark-0.15-mobile` fork branch uses **gnark 0.15.0 / gnark-crypto 0.20.1** for Zolana compatibility. Use circuit/key files generated by the versions for your target. Standard gnark hints, including `api.ToBinary`, are registered in the shipped modules. Custom solver hints must be registered before rebuilding. This integration supports Groth16 over BN254; other curves and PLONK are not exposed.
